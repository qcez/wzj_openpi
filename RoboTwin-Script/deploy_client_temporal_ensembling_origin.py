...
_ACTION_HISTORY_LEN = 10000
_ACTION_CHUNK_SIZE_ORIGIN = 50  # 模型一次推理的步数
_ACTION_DIM = 14

_Action_History = np.zeros([
    _ACTION_HISTORY_LEN,                          # 批次起点
    _ACTION_HISTORY_LEN + _ACTION_CHUNK_SIZE_ORIGIN, # 预留缓冲
    _ACTION_DIM                                    # 动作维度
])
_Action_Infered_Sign = np.zeros(_ACTION_HISTORY_LEN)
_schedule_cursor = 0
#====================== 异步推理函数 ============================
def clear_action_buffer(): # 清空缓存
    global _schedule_cursor, _control_t, _Action_History, _Action_Infered_Sign
    with _action_lock:
        _schedule_cursor = 0
        _control_t = 0
        _Action_History[:] = 0
        _Action_Infered_Sign[:] = 0

def temporal_ensembled_action(current_time):
    """取当前时刻所有已推断的动作softmax加权平均"""
    if current_time >= _Action_History.shape[0]:
        return None
    with _action_lock:
    ...
    # 加权平均
    exp_weights = np.exp(-0.2 * np.arange(len(cur_actions)))
...
#     # indices = _Action_Infered_Sign[:current_time + 1].astype(bool)
#     valid_mask = np.zeros(_ACTION_HISTORY_LEN, dtype=bool)
#     for i in range(current_time+1):
#         if _Action_Infered_Sign[i] == 1 and i+_ACTION_CHUNK_SIZE_ORIGIN > current_time:
#             valid_mask[i] = True
#     if not np.any(valid_mask):
#         return None

#     cur_actions = cur_actions[valid_mask]
#     # 加权平均
#     k_values = current_time - np.where(valid_mask)[0]
#     exp_weights = np.exp(0.02 * k_values)
#     exp_weights = (exp_weights / exp_weights.sum())[:, None]
#     sub_action = (cur_actions * exp_weights).sum(axis=0)
...
def _enqueue_chunk_to_expected_queues(chunk, _schedule_cursor):
    """入队chunk到历史缓存"""
    global _Action_History, _Action_Infered_Sign
    
    end_pos = _schedule_cursor + len(chunk)
    if end_pos >= _Action_History.shape[1]:
        print(f"警告：Action buffer溢出! {_schedule_cursor} → {end_pos}, Capacity={_Action_History.shape[1]}")
        
        # 如果即将填满整个缓冲区，强制重置并清空
        if _schedule_cursor >= _ACTION_HISTORY_LEN - (_ACTION_CHUNK_SIZE_ORIGIN + 10):
            print("触发 Buffer 重置机制...")
            clear_action_buffer()
            _schedule_cursor = 0
    ...
    with _action_lock:
        _Action_History[_schedule_cursor, _schedule_cursor:end_pos, :] = np.array(chunk)
...
class InferController:
    ...
    def move_initial(self, position_state):
        ...
        try:
            # 左臂
            self.left_arm.move_j(left_arm_position)
            self.left_gripper.move_gripper(width=left_gripper_position, force=1.0)
            # 右臂
            self.right_arm.move_j(right_arm_position)
            self.right_gripper.move_gripper(width=right_gripper_position, force=1.0)
    ...
    def run(self, ros_operator):
        ...
        # 回到初始位置
        self.move(np.concatenate((self.LEFT_INIT_POSITION, self.RIGHT_INIT_POSITION)))
    ...
    def _action_producer_loop(self, ros_operator):
        ...
        while not _action_stop_event.is_set() and not rospy.is_shutdown():
            ...
            try:
                ...
                _enqueue_chunk_to_expected_queues(action_chunk, _schedule_cursor)
            except Exception as e:
