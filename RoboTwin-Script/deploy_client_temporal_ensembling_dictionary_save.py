...
# ==================== 全局计数器 (跨 Episode 累积) ====================
episode_idx = 0
episode_idx_dict = {"success": 0, "fail": 0, "human": 0}
success_count = 0
total_count = 0
...
def save_data(images, actions, states, success, folder_type, args):
    ...
    global episode_idx, episode_idx_dict 
    ...
    for i, action in enumerate(actions):
        if i < len(images):
            img_tuple = images[i]
            # img_tuple 来自 ros_operator.get_frame() -> (front, left, right, ...)
            # get_frame 返回顺序是 (img_front, img_left, img_right, ...)
            # img_high = img_tuple[0]
            # img_left = img_tuple[1]
            # img_right = img_tuple[2]
    ...
    # 创建路径
    # idx = episode_idx + getattr(args, 'episode_idx_offset', 0)


    # base_offset = getattr(args, 'episode_idx_offset', 0)
...
class InferController:
    ...
    def run(self, ros_operator, args):
        global episode_idx, success_count, total_count, stop_signal
...
def get_arguments():
    ...
    parser.add_argument('--output_dir', action='store', type=str, 
                        default='/home/agilex/cobot_magic/collect_data/data/folding_cloth_3_14_infer/', required=False)
    # parser.add_argument('--episode_idx_offset', action='store', type=int, default=0, required=False)
