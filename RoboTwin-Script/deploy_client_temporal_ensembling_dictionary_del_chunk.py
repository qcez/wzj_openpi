...
def clear_action_buffer(): # 清空缓存
    global _control_t, _action_chunks, _buffer_no
    with _action_lock:
        _control_t = 0
...
def temporal_ensembled_action(current_time):
    ...
    with _action_lock:
        ...
        # if exist one chunk with cursor < t - 50, delete all before t chunk. first chunk finished up
        if before_keys[0] + _ACTION_CHUNK_SIZE_ORIGIN < current_time:
            # remove all chunks except last one, last chunk is now new base chunk
            for k in before_keys[:-1]:
                del _action_chunks[k]

        print(f"[action_chunks] t={current_time} after={sorted(_action_chunks.keys())}")

        # 只提取每个chunk在当前时刻对应的一帧动作（14维）
        # cursor表示chunk起始时刻，当前应取索引 current_time - cursor
        sorted_items = sorted(_action_chunks.items(), key=lambda x: x[0])
        aligned_actions = []
        for cursor, chunk in sorted_items:
            idx = current_time - cursor
            if 0 <= idx < len(chunk):
                aligned_actions.append(chunk[idx])

    if not aligned_actions:
        return None

    cur_actions = np.asarray(aligned_actions, dtype=np.float32)  # [num_chunks, 14]
    exp_weights = np.exp(0.3 * np.arange(len(cur_actions), dtype=np.float32))
    exp_weights = exp_weights / exp_weights.sum()
    sub_action = (cur_actions * exp_weights[:, None]).sum(axis=0)
    return sub_action
