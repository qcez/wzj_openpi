...
def parse_args_and_config():
    ...
    parser.add_argument("--env_seed", nargs='*', type=str, default=[])
    ...
    # Handle env_seed
    if args.env_seed == [] or args.env_seed == ['None']:
        env_seed = None
    else:
        env_seed = [int(x) for x in args.env_seed]

    config['device'] = f"cuda:{os.environ.get('LOCAL_RANK', 0)}"
    return config, args.file_name, args.task_names, args.seed_base, env_seed, args.gpu_num, args.batch_size_per_gpu

import subprocess
import json

def run_sapien_distributed_subprocess(usr_args, gpu_num = 1, batch_size_per_gpu = 8, seed_base = 100000, env_seed = None, file_name = None, noise_level = 0.1):
...
try:
    ...
    while time.time() - start_time < 3600:
        ...
        for wrapper in wrapper_envs:
            ...
            # 计算episode_length：测试结束时的step数
            episode_length = len(trajectory)
            # 为每个step添加episode_length
...
if __name__ == "__main__":
    ...
    Sapien_TEST()
    usr_args, base_file_name, task_names, seed_base, env_seed, gpu_num, batch_size_per_gpu = parse_args_and_config()
    print(usr_args)
    ...
    for task_name in task_names:
        ...
        run_sapien_distributed_subprocess(usr_args, gpu_num, batch_size_per_gpu, seed_base=seed_base, env_seed=env_seed, file_name=file_name)
