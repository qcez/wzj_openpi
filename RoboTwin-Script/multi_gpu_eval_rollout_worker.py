...
def run_rollout(args, config):
    ...
    try:
        for base_seed in args.env_seed:
            ...
            while task_id < args.batch_size:
                ...
                print(f"[{time.strftime('%Y-%m-%d_%H:%M:%S', time.localtime())}][Worker:{task_name}] episode {task_id + 1}/{args.batch_size} seed={seed}")
                ...
                # Save every save_freq tasks or at the end
                if task_id % args.save_freq == 0 or task_id == args.batch_size:
                    pickle_data = {
                        'time': current_time,
                        'results': results,
                    }

                    save_dir = f"rollout_result/{task_name}/{config.get('policy_name', 'default')}/{config.get('task_config', 'default')}/{config.get('ckpt_setting', 'default')}/{current_time}"
                    os.makedirs(save_dir, exist_ok=True)

                    pickle_file = os.path.join(save_dir, f"{task_name}_{args.file_name}_{batch_index}.pkl")
                    import pickle
                    with open(pickle_file, "wb") as f:
                        pickle.dump(pickle_data, f)
                    print(f"Rollout data saved to {pickle_file}")

                    # Accumulate total results (without trajectory to save memory)
                    total_results.extend([{k: v for k, v in r.items() if k != 'trajectory'} for r in results])
    ...
    succ_num = sum(1 for r in total_results if r["result"])
    print(f"\n[{time.strftime('%Y-%m-%d_%H:%M:%S', time.localtime())}][Worker:{task_name}] Done in {elapsed:.0f}s — {succ_num}/{len(total_results)} success")

    # Save total instructions JSON
    if total_results:
        ...
        instructions_data = {
            ...
            "success_rate": f"{round(succ_num / len(total_results) * 100, 1)}%" if total_results else "0%",
            "elapsed_time": elapsed,
            "test_records": [{"seed": r["seed"], "success": r["result"], "instruction": r["instruction"], "task_id": r["task_id"]} for r in total_results]
        }
        ...
        print(f"Total rollout instructions saved to {instructions_json_path}")
...
def main():
    ...
    parser.add_argument("--file_name", type=str, default="rollout_pkl")
    parser.add_argument("--save_freq", type=int, default=10, help="Save frequency for rollout mode")
