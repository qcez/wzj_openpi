...
def main(usr_args):
    ...
    st_seed, suc_num, test_records = eval_policy(task_name,
                                   TASK_ENV,
                                   args,
                                   model,
                                   st_seed,
...
def eval_policy(task_name,
    ...
    Args:
        use_saved_instructions: 是否使用已保存的instruction而不是随机生成
        instructions_json_path: 已保存的instruction JSON文件路径
    """
    print(f"\033[34mTask Name: {args['task_name']}\033[0m")
    print(f"\033[34mPolicy Name: {args['policy_name']}\033[0m")
    
    # 加载已保存的instruction（如果提供）
    ...
    if use_saved_instructions and instructions_json_path:
        try:
            ...
            saved_instructions = {rec["seed"]: rec["instruction"] for rec in instructions_data["test_records"]}
            logging.info(f"✅ Loaded {len(saved_instructions)} saved instructions from {instructions_json_path}")
        except Exception as e:
            logging.warning(f"Failed to load saved instructions: {e}")
            saved_instructions = None
    ...
    now_id = 0
    succ_seed = 0  # 【重要】记录通过Expert Check的成功测试次数
    suc_test_seed_list = []
    ...
    while succ_seed < test_num:
        ...
        # 【修改】优先使用已保存的instruction，否则随机生成
        if saved_instructions and now_seed in saved_instructions:
            instruction = saved_instructions[now_seed]
            logging.info(f"Using saved instruction for seed {now_seed}")
        else:
    ...
    # 返回test_records供上层函数使用
    return now_seed, TASK_ENV.suc, test_records
...
def parse_args_and_config():
    ...
    # 使用parse_args()而不是parse_known_args()
    ...
    logging.info(f"🔍 Final policy_name: {config.get('policy_name')}")  # 调试
