    ...
    while global_step < config.num_train_steps:
        ...
        for observation, actions in loader:
            ...
            if isinstance(actions, dict):
                # 排除索引，并在拼接时强制转换为 .float()
                action_list = [
                    v.to(device).float() if torch.is_tensor(v) else torch.tensor(v).to(device).float() 
                    for k, v in sorted(actions.items()) if k != 'dataset_indices'
                ]
                actions = torch.cat(action_list, dim=-1)
            else:
                # 如果已经是 Tensor，直接转为 float32
